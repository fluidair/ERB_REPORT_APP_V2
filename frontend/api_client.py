...
class APIClient:
    def __init__(self, base_url: str = None):
        self.base_url = base_url or os.environ.get("BACKEND_URL", "https://erb-backend.onrender.com")
    ...
    def update_user_role(self, user_id: int, new_role: str) -> bool:
        if not self.token:
            return False
        try:
            resp = self.session.put(f"{self.base_url}/admin/users/{user_id}", json={"role": new_role})
            return resp.ok
    ...
    def deactivate_user(self, user_id: int) -> bool:
        if not self.token:
            return False
        try:
            resp = self.session.put(f"{self.base_url}/admin/users/{user_id}", json={"is_active": False})
            return resp.ok
        except requests.RequestException:
            return False
    ...
    def activate_user(self, user_id: int) -> bool:
        if not self.token:
            return False
        try:
            resp = self.session.put(f"{self.base_url}/admin/users/{user_id}", json={"is_active": True})
            return resp.ok
        except requests.RequestException:
            return False
    ...
    def update_profile(self, update_data: dict) -> Tuple[bool, Any]:
        if not self.token:
            return False, {"detail": "No token set"}
        try:
            resp = self.session.put(f"{self.base_url}/profile/me", json=update_data)
            return resp.ok, resp.json()
        except requests.RequestException as e:
            return False, {"detail": str(e)}
    ...
    def resend_verification_email(self, email: str) -> Tuple[bool, Any]:
        try:
            resp = self.session.post(f"{self.base_url}/auth/resend-verification", params={"email": email})
            return resp.ok, resp.json()
        except requests.RequestException as e:
            return False, {"detail": str(e)}
